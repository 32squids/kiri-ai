...
players' two chosen cards for a turn are revealed. This module is a pure
resolution engine: given a GameState and each player's already-chosen,
already-ordered pair of cards for the turn, it returns the resulting
GameState plus a log of what happened. It does not enumerate legal
actions for a policy to choose from -- that's a separate, later concern.

Three ruling calls were made where the physical ruleset is silent,
confirmed with the game's designer before implementing:

  1. Card 2's stance-gate (Heaven/Earth Strike, Zan-tetsu, Kesa) is
     checked against the stance the player will be in *after* their own
     Card 1 resolves, not the stance they were in at the start of the
     turn. This is always knowable at selection time, since it depends
     only on the player's own Card 1 (Stance Change and the Zan-tetsu/
     Kesa post-attack swap are both unconditional and opponent-
     independent), never on the opponent's simultaneous choice. This
     permits e.g. Stance Change into a now-legal attack within one turn.
     See `stance_after_playing` and `validate_turn_plan`.

  2. When both players play a Priority-Tier-2 movement card in the same
     reveal -- Forward and Backward share a tier, so this includes any
     combination (Forward/Forward, Backward/Backward, or one of each) --
     they resolve simultaneously against each other's pre-step positions,
     with the same crossing-veto no-op rule the ruleset spells out for
     identical same-type pairs (e.g. Charge/Charge). See
     `_apply_simultaneous_movement`.

  3. Two players simultaneously playing Special Counterattack is not a
     real case: only one physical Counterattack card exists in the
     3-card special pool, dealt to at most one player. No ruling needed.

Resolution never raises for a rules-legal card that simply fails to do
anything on the board (a Charge into a wall, an Earth Strike that
whiffs) -- that's a no-op, reported as an event with a false flag, per
the ruleset's explicit "illegal movement is selectable but resolves to
nothing" model. `validate_turn_plan` raises only for what the ruleset
treats as illegal to even *select*: an unavailable card (on cooldown, or
a special already spent), playing both ends of one physical card, or a
stance-gated card whose stance requirement isn't met at the relevant
moment (see ruling 1 above).
"""
...
def validate_turn_plan(player: PlayerState, plan: TurnPlan) -> None:
    ...
    if card1 not in available:
        raise ValueError(f"{card1} is not available to {player.side} this turn")
    if card2 not in available:
        raise ValueError(f"{card2} is not available to {player.side} this turn")
    ...
    if required1 is not None and player.stance is not required1:
        raise ValueError(f"{card1} requires {required1} stance; {player.side} is in {player.stance}")
...
def _is_move_legal(mover_side: Side, start: int, destination: int, opponent_ref_position: int) -> bool:
    """
    ...
    their square en route, is not.
    """
    if not (BOARD_MIN <= destination <= BOARD_MAX):
        return False
    d = mover_side.forward_direction
    if (destination - opponent_ref_position) * d > 0:
        return False
...
def _apply_single_movement(state: GameState, side: Side, card: Card, opponent_ref: int) -> Tuple[GameState, List[Event]]:
    ...
    destination = player.position + _move_delta(card, side)
    if _is_move_legal(side, player.position, destination, opponent_ref):
        moved, final_position = True, destination
    else:
        moved, final_position = False, player.position
...
def _apply_simultaneous_movement(
    ...
    player_a, player_b = state.player(side_a), state.player(side_b)
    cand_a = player_a.position + _move_delta(card_a, side_a)
    cand_b = player_b.position + _move_delta(card_b, side_b)
    ...
    if legal_a and legal_b:
        ...
        sign_after = _sign(cand_a - cand_b)
        if sign_before != 0 and sign_after != 0 and sign_before != sign_after:
            crossing = True

    moved_a = legal_a and not crossing
    moved_b = legal_b and not crossing
...
def _resolve_solo_attack(state: GameState, side: Side, card: Card) -> Tuple[GameState, List[Event]]:
    ...
    if card is Card.SPECIAL_COUNTERATTACK:
        events.append(AttackEvent(side=side, card=card, target_squares=frozenset(), hit=False, countered=False))
    ...
    events.append(AttackEvent(side=side, card=card, target_squares=targets, hit=hit, countered=False))
...
def _resolve_mutual_attack(
    ...
    if counter_side is not None:
        ...
        events.append(AttackEvent(side=attacker_side, card=attacker_card, target_squares=targets, hit=False, countered=would_hit))
        events.append(AttackEvent(side=counter_side, card=Card.SPECIAL_COUNTERATTACK, target_squares=frozenset(), hit=would_hit, countered=False))
    ...
    events.append(AttackEvent(side=side_a, card=card_a, target_squares=targets_a, hit=hit_a_lands, countered=False))
    events.append(AttackEvent(side=side_b, card=card_b, target_squares=targets_b, hit=hit_b_lands, countered=False))
...
def resolve_turn(state: GameState, plan_one: TurnPlan, plan_two: TurnPlan) -> Tuple[GameState, List[Event]]:
    """
    ...
    same way. Returns the resulting state and the ordered log of events.
    """
    if state.is_over:
